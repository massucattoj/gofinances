...
import { useBottomTabBarHeight } from '@react-navigation/bottom-tabs'; // Pegar a altura do menu pra descontar na lista
...
export function Resume() {
	...
	async function loadData() {
		...
		const expensives = responseFormatted.filter((exp : TransactionData) =>
			exp.type === 'negative' &&
			new Date(exp.date).getMonth() === selectedDate.getMonth() &&
			new Date(exp.date).getFullYear() === selectedDate.getFullYear()
		);

		const expensiveTotal = expensives.reduce((acc: number, exp: TransactionData) => {
